...
# This script also supports executing dump and reduced SQL files obtained from pstress trial instead of running pstress trials.
# To enable this mode, set the following variables in your configuration file:
...
#   SQL_FILES_KILL_AND_RESTART_SERVER=0  # Set to 1 to kill and restart server after SQL_FILES_TIMEOUT seconds post triggereing the SQL file/s execution. (default: 0)

#
# The SQL files directory structure should be as generated by the replay_test.sh script when ran in SQL FILES MODE:
#   ${SQL_FILES_DIR}/${SQL_FILES_STEP_NUM}/
#     - ts"${SQL_FILES_STEP_NUM}".sql (optional). tablespace creation SQL file.
#     - dump_pstress${SQL_FILES_STEP_NUM}.sql (required if SQL_FILES_SINGLE_THREADED=0). dump file obtained from the prioir trial.
#     - workaround.sql (optional). workaround SQL files, containg SQL queries if any to be executed before the reduced_* SQL files execution.
...
# Set default values for SQL file execution mode if not set
if [ -z "${EXECUTE_SQL_FILES_MODE:-}" ]; then
  EXECUTE_SQL_FILES_MODE=0
fi
if [ ${EXECUTE_SQL_FILES_MODE} -eq 1 ]; then
  TRIALS=1
  if [ -z "${SQL_FILES_DIR:-}" ]; then
    SQL_FILES_DIR=${HOME}/pstress_sql_files
  fi
  if [ -z "${SQL_FILES_STEP_NUM:-}" ]; then
    echoit "Error: EXECUTE_SQL_FILES_MODE=1 but SQL_FILES_STEP_NUM is not set in configuration file"
    exit 1
  fi
  SQL_FILES_STEP_DIR=$(realpath "${SQL_FILES_DIR}/${SQL_FILES_STEP_NUM}" 2>/dev/null)
  if [ ! -d "$SQL_FILES_STEP_DIR" ]; then
    echoit "Error: SQL files directory ${SQL_FILES_STEP_DIR} does not exist"
    exit 1
  fi
  if [ -z "${SQL_FILES_NUMBER_OF_TIMES:-}" ]; then
    SQL_FILES_NUMBER_OF_TIMES=1
  fi
  if [ -z "${SQL_FILES_SINGLE_THREADED:-}" ]; then
    SQL_FILES_SINGLE_THREADED=0
  fi
  if [ -z "${SQL_FILES_TIMEOUT:-}" ]; then
    SQL_FILES_TIMEOUT=60
  fi
  if [ -z "${SQL_FILES_KILL_AND_RESTART_SERVER:-}" ]; then
    SQL_FILES_KILL_AND_RESTART_SERVER=0
  fi
fi
...
if [ ${EXECUTE_SQL_FILES_MODE} -eq 1 ]; then
  if [ ${SQL_FILES_SINGLE_THREADED} -eq 1 ]; then
    echoit "MODE: Single threaded SQL files execution"
  else
    echoit "MODE: Execute SQL files mode"
  fi
...
# Function to kill and restart the server
kill_and_restart_server(){
  if [[ ${PXC} -eq 0 && ${GRP_RPL} -eq 0 ]]; then
    MPID=( $(ps -ef | grep socket=${SOCKET} | grep ${RUNDIR} | grep -v grep | awk '{print $2}') )
    echoit "Killing server (pid: ${MPID}) for restart..."
    kill_server 9 ${MPID}
    sleep 2

    if [ -z "${SERVER_RESTART_CMD:-}" ]; then
      echoit "ERROR: SERVER_RESTART_CMD is not set. Cannot restart server."
      echoit "ERROR: Check if the server setup was not done properly."
      return 1
    fi

    echoit "Restarting server..."

    ${SERVER_RESTART_CMD} > ${RUNDIR}/${TRIAL}/log/master.err 2>&1 &
    MPID="$!"

    echoit "Waiting for server (pid: ${MPID}) to restart..."
    for X in $(seq 0 ${MYSQLD_START_TIMEOUT}); do
      sleep 1
      if ${BASEDIR}/bin/mysqladmin -uroot -S${SOCKET} ping > /dev/null 2>&1; then
        echoit "Server restarted successfully"
        return 0
      fi
      if [ $X -eq ${MYSQLD_START_TIMEOUT} ]; then
        echoit "ERR: Server failed to restart after ${MYSQLD_START_TIMEOUT} seconds"
        return 1
      fi
    done
  fi
...
# Function to execute dump and SQL files
execute_sql_files(){
  echoit "Executing SQL files for step ${SQL_FILES_STEP_NUM} from ${SQL_FILES_STEP_DIR}"
  local socket=${SOCKET}
  if [[ ${PXC} -eq 1 || ${GRP_RPL} -eq 1 ]]; then
    socket=${SOCKET1}
  fi

  # Execute mysql queries from a file.
  execute_queries_from_file() {
    local file=$1
    if ! "${BASEDIR}/bin/mysql" -uroot -S"${socket}" -f < "$file"; then
      echoit "ERROR: Failed to execute $file file"
      return 1
    fi
  }

  set_global_variable() {
    if ! "${BASEDIR}/bin/mysql" -uroot -S"${socket}" -e"set global ${1}" ; then
      echoit "ERROR setting global variable ${1}"
      return 1
    fi
  }

  local ts_file=${SQL_FILES_STEP_DIR}/ts${SQL_FILES_STEP_NUM}.sql
  if [ -f "$ts_file" ]; then
    echoit "Executing $ts_file file"
    execute_queries_from_file "$ts_file" || return 1
  fi

  if [ ${PXC} -eq 1 ]; then
    set_global_variable pxc_strict_mode=DISABLED || return 1
  fi

  local dump_file=${SQL_FILES_STEP_DIR}/dump_pstress${SQL_FILES_STEP_NUM}.sql
  if [ -f "$dump_file" ]; then
    echoit "Executing $dump_file file"
    execute_queries_from_file "$dump_file" || return 1
  else
    echoit "Warning: Dump file dump_pstress${SQL_FILES_STEP_NUM}.sql not found in ${SQL_FILES_STEP_DIR}"
  fi
  ...
  local workaround_file=${SQL_FILES_STEP_DIR}/workaround.sql
  if [ -f "$workaround_file" ]; then
    echoit "Executing $workaround_file file"
    execute_queries_from_file "$workaround_file" || return 1
  fi

  # Execute reduced_* SQL files
  echoit "Executing step SQL files (reduced_*)"

  if [ $SQL_FILES_NUMBER_OF_TIMES -eq 0 ]; then
    echoit "Skipping executing the step sqls (number_of_times=0)"
    return 0
  fi

  for i in $(seq 1 $SQL_FILES_NUMBER_OF_TIMES); do
    pids=()
    for sql_file in ${SQL_FILES_STEP_DIR}/reduced_*; do
      if [ ! -f "$sql_file" ]; then
        continue
      fi
      echoit "Executing SQL file: $(basename $sql_file)"
      $BASEDIR/bin/mysql -uroot -S $socket -f >/dev/null 2>&1 < $sql_file &
      pid=$!
      pids+=($pid)

      if [ $SQL_FILES_SINGLE_THREADED -eq 1 ]; then
        echoit "Started SQL file execution process: ${pid}"
        sleep $SQL_FILES_TIMEOUT
        if [ $SQL_FILES_KILL_AND_RESTART_SERVER -eq 1 ]; then
          echoit "Killing and restarting server (single-threaded mode)"
          kill_and_restart_server
        fi
        if kill -0 "$pid" 2>/dev/null; then
          kill "$pid" && echoit "Killed SQL file execution process $pid"
        fi
      fi
    done

    if [ ${#pids[@]} -eq 0 ]; then
      echoit "No reduce_d* SQL files found in ${SQL_FILES_STEP_DIR}"
      break
    fi

    if [ $SQL_FILES_SINGLE_THREADED -eq 0 ]; then
      echoit "Started ${#pids[@]} SQL file execution processes: ${pids[*]}"
      # Start a timer in background
      (
        sleep $SQL_FILES_TIMEOUT
        if [ $SQL_FILES_KILL_AND_RESTART_SERVER -eq 1 ]; then
          echoit "Timeout reached. Killing and restarting server"
          kill_and_restart_server
        fi
        echoit "Killing remaining SQL execution jobs..."
        for pid in "${pids[@]}"; do
          if kill -0 "$pid" 2>/dev/null; then
            kill "$pid" && echoit "Killed SQL file execution process $pid"
          fi
      ...
      if [ $SQL_FILES_KILL_AND_RESTART_SERVER -eq 1 ]; then
        # Wait for timeout watcher to complete (it may restart the server)
        wait $timeout_pid 2>/dev/null
      else
        # Kill timeout watcher if all jobs finished early
        kill "$timeout_pid" 2>/dev/null
      fi
    fi

    echoit "Loop $i of $SQL_FILES_NUMBER_OF_TIMES completed"
  done

  echoit "SQL files execution completed for step ${SQL_FILES_STEP_NUM}"
  return 0
}
...
pstress_test(){
  ...
  if [ ${EXECUTE_SQL_FILES_MODE} -eq 1 ]; then
    echoit "====== SQL FILES STEP #${SQL_FILES_STEP_NUM} ======"
  else
    echoit "====== TRIAL #${TRIAL} ======"
  fi
  ...
  if [[ ${PXC} -eq 0 && ${GRP_RPL} -eq 0 ]]; then
    ...
    # Store CMD for potential server restart during SQL file execution
    SERVER_RESTART_CMD="$CMD"
...
EOF
    ...
    if [[ ${TRIAL} -eq 1 || $REINIT_DATADIR -eq 1 ]]; then
      ...
      if [ -n "${COMPONENT_NAME}" ]; then
        ...
        if [ "${COMPONENT_NAME:-}" == "component_masking_functions" ]; then
          echoit "Setting up the component : ${COMPONENT_NAME}"
      ...
      if [ -n "${SQL_FILE}" ]; then
        echoit "Executing the queries from file ${SQL_FILE}"
        ${BASEDIR}/bin/mysql -uroot -S${SOCKET} -f < ${SCRIPT_PWD}/${SQL_FILE} > ${WORKDIR}/sql_file_step_${TRIAL}.result 2>&1
      fi
    fi

    # Check if SQL file execution mode is enabled
    if [ ${EXECUTE_SQL_FILES_MODE} -eq 1 ]; then
      echoit "SQL file execution mode enabled. Will execute the configured dump and SQL files instead of running pstress."
      execute_sql_files
      TIMEOUT_REACHED=0
      MPID=( $(ps -ef | grep socket=${SOCKET} | grep ${RUNDIR} | grep -v grep | awk '{print $2}') )
    else
      # Normal pstress execution mode
      if [[ ${PXC} -eq 0 && ${GRP_RPL} -eq 0 ]]; then
  ...
  if [ ${ISSTARTED} -eq 1 -a ${TRIAL_SAVED} -ne 1 ]; then  # Do not try and print pstress log for a failed mysqld start
    if [ ${EXECUTE_SQL_FILES_MODE} -eq 0 ]; then
      echoit "pstress run details:$(grep -i 'SUMMARY.*queries failed' ${RUNDIR}/${TRIAL}/*.sql ${RUNDIR}/${TRIAL}/*.log 2>/dev/null | sed 's|.*:||')"
    else
      echoit "SQL files execution mode: Trial ${TRIAL} completed"
    fi
  fi
