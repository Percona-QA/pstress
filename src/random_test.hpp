...
struct FK_table : Table {
  ...
  /* conustructor used for load_metadata */
  ...
  ForeignKeyAction getRandomForeignKeyAction(Table *table) {
    /* if a table has a virtual generated column and if any of the base column
     * column use RESTRICT */
    ...
    int randomValue = rand_int(static_cast<int>(ForeignKeyAction::NO_ACTION));
